...
// Dirección compuesta de una entidad
function dir(e) { return [e.address, e.town, e.state, e.country].filter((x) => x && String(x).trim()).join(', '); }

// Resuelve el CLIENTE a partir del NIT: entidad padre + sus sedes (sub-entidades).
// Excluye SIEMPRE la entidad raíz (id 0) por seguridad. Devuelve null si no existe.
//  -> { entidad: {id, nombre, direccion}, sedes: [{id, nombre, direccion}] }
async function resolverCliente(nit) {
  const objetivo = normNit(nit);
  if (!objetivo || objetivo.length < 5) return null;
  const r = await api('GET', '/Entity?range=0-2000&expand_dropdowns=false');
  const todas = await r.json().catch(() => null);
  if (!Array.isArray(todas)) return null;

  // entidades (NO raíz) cuyo texto contenga el NIT
  const matches = todas.filter((e) => Number(e.id) !== 0
    && Object.values(e).some((v) => typeof v === 'string' && normNit(v).includes(objetivo)));
  if (!matches.length) return null;

  // el "cliente" es el match más arriba en el árbol (menor level / menor id)
  matches.sort((a, b) => (Number(a.level || 99) - Number(b.level || 99)) || (Number(a.id) - Number(b.id)));
  const padre = matches[0];

  // Sedes seleccionables = la entidad padre + sus sub-entidades, cada una con su dirección.
  // El id elegido será el entities_id del ticket.
  const ubicaciones = [{ id: padre.id, nombre: padre.completename || padre.name, direccion: dir(padre) }];
  for (const e of todas.filter((e) => String(e.entities_id) === String(padre.id))) {
    ubicaciones.push({ id: e.id, nombre: e.completename || e.name, direccion: dir(e) });
  }

  return { nombre: padre.completename || padre.name, padreId: padre.id, ubicaciones };
}
...
// Lista las sub-entidades directas (hijos) de una entidad, con su dirección
async function listarSubentidades(entityId) {
  const r = await api('GET', '/Entity?range=0-1000&expand_dropdowns=false');
  const lista = await r.json().catch(() => null);
  if (!Array.isArray(lista)) return [];
  return lista
    .filter((e) => String(e.entities_id) === String(entityId))
    .map((e) => ({
      id: e.id,
      nombre: e.completename || e.name || '',
      direccion: [e.address, e.town, e.state, e.country].filter((x) => x && String(x).trim()).join(', '),
    }));
}

// Devuelve las ubicaciones crudas (todos los campos) de una entidad
async function getUbicacionesRaw(entityId) {
  const r = await api('GET', '/Location?range=0-1000&expand_dropdowns=false');
  const lista = await r.json().catch(() => null);
  if (!Array.isArray(lista)) return [];
  return lista.filter((l) => String(l.entities_id) === String(entityId));
}

// Lista las ubicaciones (sedes/direcciones) de una entidad -> [{ id, nombre, direccion }]
async function listarUbicaciones(entityId) {
  const raw = await getUbicacionesRaw(entityId);
  return raw.map((l) => ({
    id: l.id,
    nombre: l.completename || l.name || '',
    // Dirección compuesta a partir de los campos de la ubicación
    direccion: [l.address, l.town, l.state, l.country].filter((x) => x && String(x).trim()).join(', '),
  }));
}
...
// Restringe la sesión al árbol de una entidad (la entidad + sus sub-entidades).
// Es el mecanismo de AISLAMIENTO: tras esto, las consultas solo ven esa entidad.
// Devuelve false si no se pudo (para fallar en seguro y no filtrar datos de otros).
async function restringirAEntidad(entityId) {
  try {
    const r = await api('POST', '/changeActiveEntities', { entities_id: Number(entityId), is_recursive: true });
    return r.ok;
  } catch { return false; }
}

// Consulta un ticket por número, AISLADO al árbol de la entidad indicada.
// Devuelve null si no existe o si NO pertenece a esa entidad (no se revela nada de otros).
async function estadoTicket(numero, entityId) {
  const id = String(numero).replace(/[^0-9]/g, '');
  if (!id) return null;
  if (entityId != null && entityId !== '') {
    if (!(await restringirAEntidad(entityId))) return null; // fail-closed
  }
  const r = await api('GET', '/Ticket/' + id);
  if (!r.ok) return null; // 404/403/401 -> no existe o fuera de su entidad
  const d = await r.json().catch(() => null);
...
async function listarTicketsAbiertos(entityId) {
  if (!(await restringirAEntidad(entityId))) return []; // fail-closed: si no aíslo, no devuelvo nada
  const r = await api('GET', '/Ticket?range=0-300&expand_dropdowns=false&order=DESC&sort=19');
  const lista = await r.json().catch(() => null);
  if (!Array.isArray(lista)) return [];
  return lista
    .filter((t) => [1, 2, 3, 4].includes(Number(t.status))) // abiertos; la sesión ya está aislada a su árbol
...
module.exports = {
  ...
  buscarEntidadPorNit, resolverCliente, listarUbicaciones, getUbicacionesRaw, listarSubentidades, crearTicket, estadoTicket, adjuntarDocumento, getEntidad,
