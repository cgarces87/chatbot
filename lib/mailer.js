...
async function enviar({ to, subject, text, html, cc }) {
  ...
  const info = await cliente().sendMail({ from, to, cc, subject, text, html: html || undefined });
  return { id: info.messageId, aceptados: info.accepted, rechazados: info.rejected };
}
